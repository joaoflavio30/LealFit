...
import com.joaoflaviofreitas.lealfit.data.db.repository.WorkoutDetailsRepositoryImpl
...
object DataModule {
    ...
    fun providesWorkoutRepository(
        ...
        WorkoutRepositoryImpl(makeWorkoutDTOMapper(), db, auth)
    ...
    @Provides
    @Singleton
    fun providesWorkoutDetailsRepository(
        db: FirebaseFirestore,
        auth: FirebaseAuth,
    ): WorkoutDetailsRepository =
        WorkoutDetailsRepositoryImpl(db, auth, makeWorkoutMapper(), makeWorkoutDTOMapper())
}